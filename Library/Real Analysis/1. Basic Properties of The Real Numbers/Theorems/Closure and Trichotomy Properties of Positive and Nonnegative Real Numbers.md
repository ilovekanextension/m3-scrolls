...
**Proof of First Statement.** Since $a>0$ and $b>0$, we have $a+b>0+b$, so $a+b>b$. Since $<$ is a strict total order, by definition it is transitive, so since $a+b>b$ and $b>0$ we have $a+b>0$. $\blacksquare$
...
**Proof of Third Statement.** Since $a>0$ and $b>0$, we have $a\cdot b>0\cdot b$, so by [[Multiplications with Ring Zeros]] $a\cdot b>0$. $\blacksquare$
...
The first point is equivalent to $a>0$. To show that the second is equivalent to $-a>0$, observe that
$$\begin{align}
a<0 & \iff a+(-a)<0+(-a) \\
 & \iff 0<-a \\
 & \iff-a>0.\blacksquare
