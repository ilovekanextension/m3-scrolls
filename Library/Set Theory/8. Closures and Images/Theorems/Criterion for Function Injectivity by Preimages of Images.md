...
- **Backward Implication.** Suppose that for all sets $S\subseteq A$, $$S=f^{-1}[f[S]].$$Suppose $x,y\in A$, and suppose that $f(x)=f(y)$. Since $x\in A$, by [[Criterion for Singletons as Members of Power Sets]] $\{x\}\subseteq A$, so by assumption $$\{x\}=f^{-1}[f[\{x\}]].$$Since $f(x)=f(y)$ and $f(x)\in f[\{x\}]$, we must have $f(y)\in f[\{x\}]$, so $y\in f^{-1}[f[\{x\}]]$. This means $y\in\{x\}$, so $y=x$. $\blacksquare$
